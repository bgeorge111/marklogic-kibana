...
#####################################################################
# Install Filebeat agent. As internet connectivity not to be assumed
# the installation files are expected to be copied already to /tmp
# This can be a yum install too 
# TODO: The hardcoded information should be changed later and as a 
# service
#####################################################################
info "Installing and configuring Filebeat agent." 
sudo gunzip /tmp/filebeat-7.17.7-linux-x86_64.tar.gz
sudo tar -xvf /tmp/filebeat-7.17.7-linux-x86_64.tar --directory /tmp
sudo nohup /tmp/filebeat-7.17.7-linux-x86_64/filebeat --path.config /tmp/filebeat-error &
sudo nohup /tmp/filebeat-7.17.7-linux-x86_64/filebeat --path.config /tmp/filebeat-access &
info "Completed installation and configuration of Filebeat agent."
